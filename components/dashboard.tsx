...
export function Dashboard({ transcriptions, onAddTranscription, onSelectTranscription }: DashboardProps) {
      ...
      if (file) {
        ...
        setTimeout(() => {
          onAddTranscription({
            ...
            preview: "This is a simulated transcription of your uploaded audio file...",
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="container mx-auto px-6 py-8">
        <div className="mb-8">
          ...
          {filteredTranscriptions.length === 0 && searchQuery === "" ? (
            <div className="text-center py-16">
              <h2 className="text-xl font-semibold mb-2">Welcome, whisperer!</h2>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-slate-900 mb-1">{transcription.title}</h3>
                      <p className="text-sm text-muted-foreground mb-2 line-clamp-2">{transcription.preview}</p>
        ...
        <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 flex flex-col gap-3 w-full max-w-sm px-6">
          <Button variant="outline" size="lg" onClick={handleUploadVoiceNote} className="w-full bg-transparent">
            <Upload className="w-4 h-4 mr-2" />
            Upload Voice Note
          </Button>
          <Button size="lg" onClick={handleNewWhisper} className="w-full bg-slate-900 hover:bg-slate-800">
      ...
      {/* Recording Modal */}
      {showRecordingModal && (
        <RecordingModal onClose={() => setShowRecordingModal(false)} onSave={onAddTranscription} />
      )}
    </div>
  )
}
